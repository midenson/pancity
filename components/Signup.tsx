  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        localStorage.setItem("user_data", JSON.stringify(storageData.user_data));
    ...
    <div className="w-full max-w-[430px] mx-auto min-h-screen bg-white text-black flex flex-col font-sans relative overflow-hidden">
      <div className="flex-1 overflow-y-auto px-5 pb-40 [&::-webkit-scrollbar]:hidden">
        ...
        <div className="mb-6">
          <svg width="50" height="50" viewBox="0 0 50 50" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M18 18L22 22M28 22L32 18M18 32L22 28M28 28L32 32" stroke="black" strokeWidth="4" strokeLinecap="round" />
          </svg>
        </div>

        <h1 className="text-[32px] font-bold tracking-tight mb-3">Personal details</h1>
        <p className="text-gray-500 text-[15px] leading-relaxed mb-8">
          Ensure you enter your correct details. You won't be able to change this once you submit.
        </p>
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm">
              ...
              <RequirementItem met={passwordReqs.capital} text="1 capital letter" />
              <RequirementItem met={passwordReqs.number} text="1 number" />
              <RequirementItem met={passwordReqs.special} text="1 special character" />
              <RequirementItem met={passwordReqs.length} text="8 characters" className="col-span-2" />
            </div>
          </div>
      ...
      <div className="absolute bottom-0 w-full bg-white border-t border-gray-100 px-5 py-4 pb-8">
        <div className="flex items-start gap-3 mb-6">
          ...
          />
          <label htmlFor="terms" className="text-gray-600 text-[14px] leading-tight">
            I acknowledge that I have read and agree to{" "}
            <span className="text-yellow-600 underline underline-offset-2 font-medium">Pancity's Agreements</span>
          </label>
        </div>
...
function RequirementItem({ met, text, className = "" }: { met: boolean; text: string; className?: string }) {
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      {met ? <CheckCircle2 className="w-4 h-4 text-green-500" /> : <XCircle className="w-4 h-4 text-gray-300" />}
      <span className={met ? "text-gray-700 font-medium" : "text-gray-400"}>{text}</span>
    </div>
  );
}